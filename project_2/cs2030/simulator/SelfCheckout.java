...
class SelfCheckout extends Server {
  ...
  public SelfCheckout serve(Customer customer) {
    Optional<Customer> nextWaitingCustomer = Optional.ofNullable(SelfCheckout.waitingCustomers.poll());

    return new SelfCheckout(this.id, this.maxQueueLength, nextWaitingCustomer.or(() -> Optional.of(customer)));
  }
